...
async function fetchMarketPrices() {
    try {
        // 1. BTC & ETH dari Binance (gratis, no key, real-time)
        ...
        updatePrice('BTC', parseFloat(btc.lastPrice), parseFloat(btc.priceChangePercent));
        updatePrice('ETH', parseFloat(eth.lastPrice), parseFloat(eth.priceChangePercent));

        // 2. EURUSD dari Frankfurter (gratis, no key)
        const eurRes = await fetch('https://api.frankfurter.dev/v1/latest?from=EUR&to=USD');
        const eurData = await eurRes.json();
        const eurusd = eurData.rates.USD;
        updatePrice('EURUSD', eurusd, null);

        // 3. XAUUSD (Gold) - menggunakan endpoint publik yang stabil
        // Alternatif yang sering dipakai (bisa diganti jika down)
        try {
            const goldRes = await fetch('https://api.metals.live/v1/spot/gold');
            const goldData = await goldRes.json();
            // metals.live biasanya return array [price]
            const xauusd = Array.isArray(goldData) ? goldData[0] : goldData.price || goldData;
            updatePrice('XAUUSD', parseFloat(xauusd), null);
        } catch (goldError) {
            // Fallback jika metals.live gagal
            console.warn('Gold API fallback...');
            document.getElementById('price-XAUUSD').textContent = 'Loading...';
        }
...
function updatePrice(symbol, price, changePercent = null) {
    ...
    // Tampilkan perubahan 24 jam (hanya crypto)
    if (changeEl && changePercent !== null) {
        const isPositive = changePercent >= 0;
        changeEl.textContent = `${isPositive ? '+' : ''}${changePercent.toFixed(2)}%`;
        changeEl.style.color = isPositive ? '#2ecc71' : '#e74c3c';
    } else if (changeEl) {
        changeEl.textContent = 'Live';
        changeEl.style.color = '#3498db';
    }
}

// Jalankan pertama kali + update setiap 15 detik
fetchMarketPrices();
